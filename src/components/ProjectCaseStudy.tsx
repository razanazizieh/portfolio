...
import { useNavigate } from 'react-router-dom';
import { Project } from '../types';
import { MOTION_CURVE_PREMIUM, motionRoles, VIEWPORT_EDITORIAL_CONFIG } from '../utils/motion';
import { PROJECTS_DATA } from '../data';
import PremiumImage from './PremiumImage';
          ...
          <div className="flex flex-col gap-4">
            ...
            {project.subtitle && (
              <motion.p
                ...
                animate={{ y: 0, opacity: 1 }}
                transition={{ duration: 0.8, ease: MOTION_CURVE_PREMIUM, delay: 0.15 }}
        ...
        {project.technicalHighlights && project.technicalHighlights.length > 0 && (
          <section className="flex flex-col gap-3 text-left">
            ...
            <ul className="flex flex-col gap-2.5 max-w-[62ch] pl-4 list-disc text-neutral-700 dark:text-neutral-300">
              {project.technicalHighlights.map((highlight, idx) => (
                <li key={idx} className="font-sans text-base sm:text-lg font-normal leading-[1.72]">
                  {highlight}
                </li>
              ))}
            </ul>
          ...
          <section className="flex flex-col gap-3 text-left">
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2.5 max-w-[62ch]">
              {project.technology.map((tech, idx) => (
                <motion.span
                  ...
                  variants={{
                    ...
                    visible: {
                      opacity: 1,
                      y: 0,
                      transition: { duration: 0.55, ease: MOTION_CURVE_PREMIUM, delay: idx * 0.02 }
                    }
                  }}
                  initial="hidden"
                  whileInView="visible"
                ...
                >
                  {tech}
                  {idx < project.technology.length - 1 && <span className="ml-6 text-neutral-400 dark:text-neutral-600 font-normal">/</span>}
                </motion.span>
              ))}
            </div>
          </section>
